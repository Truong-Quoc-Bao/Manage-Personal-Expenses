...
const service = {
  getUserAnalytics: async (userId) => {
    ...
    // KIỂM TRA: Nếu tháng trong Database (ví dụ: tháng 3) khác tháng hiện tại (tháng 5)
    if (
      ua.current_month &&
      (ua.current_month.month !== currentMonth || ua.current_month.year !== currentYear)
    ) {
      // Tạo object tháng mới với các chỉ số thu/chi = 0
      const freshMonth = {
        year: currentYear,
        month: currentMonth,
        income: 0,
      ...
      // Cập nhật lại trong Database để lần sau lấy ra không bị cũ nữa
      await repo.updateUserAnalyticsByuserId(userId, {
        $set: { current_month: freshMonth },
      });

      // Gán lại giá trị để trả về cho Frontend hiển thị ngay lập tức
      ua.current_month = freshMonth;
    }
  ...
  _updateUserAnalytics: async function (transaction, sign = 1) {
    ...
    //   if (isExpense)
    //     currentMonthUpdate.expense = Math.max(0, (ua.current_month?.expense || 0) + amt);
    ...
    // --- ĐOẠN NÀY PHẢI SỬA LẠI ---
    if (txYear === currentYear && txMonth === currentMonth) {
      // Phải dùng currentMonthUpdate.income (biến đã được reset) thay vì ua.current_month.income
      if (isIncome) {
